...
//
// In memory, test points live inside `optics` (kind 'test'); on disk they are
// kept in the Test Points JSON column so the file stays compatible with the
// Optical Table Designer's own Beam Propagation mode. See propagationModel.js.
...
export function serializePropagationsCsv(propagations) {
  ...
  for (const raw of Object.values(propagations)) {
    const p = toFileShape(raw)
    const row = [
      p.name ?? '',
...
export function parsePropagationsCsv(text) {
  ...
  const widthModeCol = header.indexOf('Width mode')
  const out = {}
  for (const line of body) {
    ...
    out[id] = normalizePropagation({
      ...
      source,
      waistMeasurements,
      widthMode: widthModeCol >= 0 ? cells[widthModeCol] : 'radius',
    })
  }
  return out
}
