// Beam Propagation — a full-page sandbox for Gaussian beam propagation.
// Standalone copy of webapp/frontend/src/components/BeamPropagationMode.jsx:
// beamPaths / elements / symbolDefs come from an uploaded designer project
// .zip instead of the live designer, and there is no "back to designer" exit.
...
// convention). optics kind: 'lens' (contributes ABCD), 'prism-pair',
// 'passthrough' (identity — used for non-lens elements imported from a designer
// path) or 'test' (a marked z position where the beam size is reported).
function makeDefaultPropagation(name = 'New propagation') {
  return {
    ...
    plotWidth: 720,
    plotHeight: 240,
    // Whether beam size is displayed / entered as a 1/e² 'radius' or 'diameter'.
    // Stored values are always radii; see WIDTH_MODES in propagationModel.js.
    widthMode: 'radius',
...
function BeamPlot({ traces, referenceTraces = [], events, testPoints, zTotal, measurements = [], width = 720, height = 240, title, widthMode = 'radius', showGrid = false, onDragOptic, onResize }) {
  ...
  const eventLayout = (() => {
    // Optic labels first, then test-point labels (indexed after the events), so
    // a test point next to a lens is bumped to its own row instead of
    // overprinting it.
    const items = [
      ...events.map(ev => {
        ...
        return { z_mm: ev.z_mm, chars: Math.max(line1.length, line2.length) }
      }),
      ...testPoints.map((tp, i) => ({ z_mm: tp.z_mm, chars: (tp.label ?? `t${i + 1}`).length })),
    ]
    const rowByIndex = new Array(items.length).fill(0)
    ...
    const order = items.map((_, i) => i).sort((a, b) => items[a].z_mm - items[b].z_mm)
    for (const i of order) {
      const half = items[i].chars * CHAR_W / 2 + 3
      const cx = xForZ(items[i].z_mm)
  ...
  const linePathOf = pts => pts.map((p, i) =>
    `${i ? 'L' : 'M'}${xOf(p.z_mm).toFixed(2)},${yOf(p.w_mm * wScale).toFixed(2)}`).join(' ')
  ...
  return (
    <svg className="prop-plot" width={W} height={H} ref={svgRef}
      ...
      <text x={14} y={PAD_T + plotH / 2} textAnchor="middle" fontSize={11} fill="var(--text-muted)"
        transform={`rotate(-90 14 ${PAD_T + plotH / 2})`}>{wSym} (mm, {wName})</text>

      {/* Test points — drag the line (or its label) horizontally to change z.
          Drawn before the optics so a lens icon at the same z wins the click. */}
      {testPoints.map((tp, i) => {
        const cx = xOf(tp.z_mm)
        const draggable = !!onDragOptic && tp.opticId != null
        const beginDrag = draggable ? (e => {
          e.stopPropagation(); e.preventDefault()
          dragRef.current = {
            opticId: tp.opticId, kind: 'test',
            startClientX: e.clientX, startClientY: e.clientY, startZ: tp.z_mm,
            axis: 'x',    // only z can change, so no need to wait for a direction
          }
        }) : undefined
        return (
          <g key={`t${i}`} onMouseDown={beginDrag}>
            <line x1={cx} y1={PAD_T} x2={cx} y2={PAD_T + plotH}
              stroke="#5cc46a" strokeDasharray="2 4" strokeWidth={1} opacity={0.75} />
            <text x={cx} y={iconTop - 4 - (eventLayout.rowByIndex[events.length + i] ?? 0) * ROW_H}
              textAnchor="middle" fontSize={10}
              fill="#5cc46a" style={draggable ? { cursor: 'ew-resize' } : undefined}>{tp.label ?? `t${i + 1}`}</text>
            {draggable && (
              <rect x={cx - 5} y={PAD_T} width={10} height={plotH}
                fill="transparent" style={{ cursor: 'ew-resize' }}>
                <title>Drag ← → to change z</title>
              </rect>
            )}
          </g>
        )
      ...
      {events.map((ev, i) => {
        ...
        const startDrag = (e, axis) => {
          ...
          dragRef.current = {
            opticId: ev.opticId,
            kind:  ev.kind,      // 'lens' | 'prism-pair' | 'passthrough'
        ...
        // Icon: horizontal → z, vertical → f / M.  Line: z only.
        const beginDrag     = draggable ? (e => startDrag(e, null)) : undefined
        const beginLineDrag = draggable ? (e => startDrag(e, 'x')) : undefined
        // The line hit-zone starts below the icon; a pass-through has no
        // icon, so its zone runs the full plot height (covering its dot).
        const hasIcon = isLens || isPrism
        const lineZoneTop = hasIcon ? PAD_T + LENS_H + 2 : PAD_T
        return (
          <g key={`e${i}`} opacity={enabled ? 1 : 0.35}>
            ...
            {isLens && !isNegative ? (
              ...
              // Concave (negative f). b-lens3 is wider than b-lens1 (9.3 vs
              // 6.4 units at the same height), so it gets a wider slot to come
              // out the same 34 px tall.
              <image href="/symbols/b-lens3.svg"
                x={cx - LENS_W / 2 - 1} y={iconTop}
                width={LENS_W + 2} height={LENS_H}
                preserveAspectRatio="xMidYMid meet" />
            ) : isPrism ? (
            ...
            {/* Drag hit-zones: an invisible strip over the dashed line (drag ← →
                to change z), and one over the icon + a bit of padding.
                On the icon, horizontal drag → z, vertical drag → f (lens) or
                M (prism); the first ~4 px of movement decides which axis is
                locked. */}
            {draggable && (
              <rect x={cx - 5} y={lineZoneTop} width={10} height={Math.max(0, PAD_T + plotH - lineZoneTop)}
                fill="transparent" style={{ cursor: 'ew-resize' }}
                onMouseDown={beginLineDrag}>
                <title>Drag ← → to change z</title>
              </rect>
            )}
            {draggable && hasIcon && (
      ...
      {measurements.map((m, gi) => (
        <g key={`m${gi}`}>
          {(m.points ?? []).filter(p =>
              ...
              const cx = xOf(p.z_mm), cy = yOf(p.w_mm * wScale)
              return m.marker === 'square' ? (
                ...
                <circle key={i} cx={cx} cy={cy} r={3.4}
                  fill={m.color} stroke="#fff" strokeWidth={0.6}>
                  <title>z = {p.z_mm.toFixed(1)} mm, {wSym} = {(p.w_mm * wScale).toFixed(3)} mm</title>
                </circle>
              )
            })}
        </g>
...
function makeDefaultMeasurements(widthMode = 'radius') {
  return {
    widthDefinition: widthMode === 'diameter' ? '1/e2_diameter' : '1/e2_radius',
    rows: [
...
// Parse pasted or uploaded text — TSV / CSV / whitespace-separated, 2 or 3
// columns (z, wx[, wy]) — into measurement rows. Lines whose first cell isn't a
// number (a header row, blank lines) are skipped, and quotes are ignored.
// Returns [] when nothing numeric was found.
...
function WaistFitModal({ propagation, onClose, onApply }) {
  const { scale: wScale, symbol: wSym } = widthModeInfo(propagation.widthMode)
  const seed = propagation.waistMeasurements ?? makeDefaultMeasurements(propagation.widthMode)
  ...
  function onTablePaste(e) {
    ...
    if (!parsed.length) { setImportError('Could not parse any numeric rows from clipboard'); return }
    setImportError(null)
    setRows(parsed)
  }

  // Load a CSV / text file into the table (replaces all rows on success).
  async function loadFile(file) {
    if (!file) return
    if (!/\.(csv|tsv|txt)$/i.test(file.name)) {
      setImportError(`"${file.name}" isn't a .csv file`); return
    }
    let text
    try { text = await file.text() } catch { setImportError(`Could not read "${file.name}"`); return }
    const parsed = parseMeasurementText(text)
    if (!parsed.length) { setImportError(`Could not parse any numeric rows from "${file.name}"`); return }
    setImportError(null)
    setRows(parsed)
  }
  ...
  function onDragOver(e) { e.stopPropagation(); if (hasFiles(e)) e.preventDefault() }
  ...
  function FitReport({ label, fit }) {
    ...
    return (
      <div className="fit-line">
        <b>{label}:</b> {wSym}₀ = {fmt(fit.w0_mm * wScale)} mm,
  ...
  const plotData = (() => {
    const scatterX = columns.wx.map((w, i) => ({ z: columns.zx[i], w: w * wScale }))
    const scatterY = columns.wy.map((w, i) => ({ z: columns.zy[i], w: w * wScale }))
    ...
    if (fitX?.ok) { zAll.push(fitX.z0_mm); wAll.push(fitX.w0_mm * wScale) }
    if (fitY?.ok) { zAll.push(fitY.z0_mm); wAll.push(fitY.w0_mm * wScale) }
    ...
    function curve(fit) {
      ...
      for (let i = 0; i <= N; i++) {
        ...
        out.push({ z, w: fit.w0_mm * wScale * Math.sqrt(1 + t * t) })
      }
      return out
    }
    ...
    return (
      <svg width={W} height={H} style={{ display: 'block', background: 'var(--panel-alt, #0002)',
        ...
        <text x={10} y={PADT + plotH / 2} textAnchor="middle" fontSize={10} fill="var(--text-muted)"
          transform={`rotate(-90 10 ${PADT + plotH / 2})`}>{wSym} (mm)</text>
        ...
        {fitX?.ok && (
          <g><line x1={xOf(fitX.z0_mm)} y1={PADT} x2={xOf(fitX.z0_mm)} y2={PADT + plotH}
              ...
              <circle cx={xOf(fitX.z0_mm)} cy={yOf(fitX.w0_mm * wScale)} r={2} fill="#4a9fff" /></g>
        )}
        {fitY?.ok && (
          <g><line x1={xOf(fitY.z0_mm)} y1={PADT} x2={xOf(fitY.z0_mm)} y2={PADT + plotH}
              ...
              <circle cx={xOf(fitY.z0_mm)} cy={yOf(fitY.w0_mm * wScale)} r={2} fill="#e05a5a" /></g>
        )}
        {/* legend */}
      ...
      <div className="modal-box modal-box-wide" onClick={e => e.stopPropagation()}
        ...
        {dragOver && (
          <div style={{
            ...
            border: '2px dashed var(--accent-bright)', fontSize: 14, fontWeight: 600,
          }}>Drop a CSV to fill the table</div>
        )}
        ...
        <p style={{ color: 'var(--text-muted)', margin: '0 0 8px 0', fontSize: 12 }}>
          ...
          Paste directly from Excel, or drop a CSV file on this dialog — 2
          columns (z, w) or 3 columns (z, wₓ, w_y). A header row is ignored.
        </p>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 6 }}>
          ...
          <span style={{ flex: 1 }} />
          <button className="small-btn" style={{ whiteSpace: 'nowrap' }} onClick={() => fileInputRef.current.click()}
            title="Fill the table from a CSV file (or just drop one on this dialog)">Upload CSV</button>
          <button className="small-btn" onClick={clearAll}>Clear</button>
          <input ref={fileInputRef} type="file" accept=".csv,.tsv,.txt,text/csv,text/plain"
            style={{ display: 'none' }}
        ...
        <div style={{ marginTop: 4 }}>
          ...
          {importError && <span style={{ color: 'var(--danger, #d66)', marginLeft: 8, fontSize: 12 }}>{importError}</span>}
        </div>
...
const BeamPropagationMode = forwardRef(function BeamPropagationMode({
  ...
  // same way (full angle in diameter mode) so size and slope stay consistent.
  const { scale: wScale, symbol: wSym, name: wName } = widthModeInfo(p?.widthMode)
  ...
  function addTestPoint() {
    const nTest = (p?.optics ?? []).filter(o => o.kind === 'test').length
    const tp = { id: crypto.randomUUID(), kind: 'test', z_mm: 0, label: 't' + (nTest + 1) }
    mutateDiscrete({ optics: [...(p?.optics ?? []), tp] })
  }
  ...
  // Only the layout is read; any propagations inside the zip are ignored. On
  // success the path picker opens straight away.
  async function uploadProjectForImport(file) {
    if (!file) return
    if (await onLoadProjectZip?.(file)) setImportOpen(true)
  }
  ...
  const compute = useMemo(() => {
    ...
    const rows = opticRows.sort((a, b) => a.z_mm - b.z_mm)
    // Test points are drawn on the plot as their own green markers.
    const testPoints = optics
      .filter(o => o.kind === 'test' && o.enabled !== false)
      .map(o => ({ opticId: o.id, z_mm: o.z_mm, label: o.label }))
  ...
  async function exportPropagationPdf() {
    ...
    const divKind = wName === 'diameter' ? 'full-angle div' : 'half-angle div'
    const beamSpec = (p.initMode === 'waist')
      ? (p.splitXY
          ? `waist ${wSym}0 (x,y) = ${p.wxWaist_mm * wScale}, ${p.wyWaist_mm * wScale} mm @ z = ${p.zxWaist_mm}, ${p.zyWaist_mm} mm`
          : `waist ${wSym}0 = ${p.wxWaist_mm * wScale} mm @ z = ${p.zxWaist_mm} mm`)
      : (p.splitXY
          ? `${wSym} (x,y) = ${p.w0x_mm * wScale}, ${p.w0y_mm * wScale} mm     ${divKind} (x,y) = ${p.divx_mrad * wScale}, ${p.divy_mrad * wScale} mrad`
          : `${wSym} = ${p.w0x_mm * wScale} mm     ${divKind} = ${p.divx_mrad * wScale} mrad`)
    const metaParts = [
      `wavelength = ${p.wavelength_nm} nm`,
      `distance = ${p.distance_mm} mm`,
      `beam size = 1/e^2 ${wName} (${wSym})`,
      beamSpec,
    ]
    ...
    const cols = p.splitXY
      ? ['Role', 'Label', 'z (mm)', 'f / M', `${wSym}_x (mm)`, `${wSym}_y (mm)`, 'Element']
      : ['Role', 'Label', 'z (mm)', 'f / M', `${wSym} (mm)`, 'Element']
    ...
    for (const r of rowsForPdf) {
      ...
      const wx = Number.isFinite(r.wx_mm) ? (r.wx_mm * wScale).toFixed(4) : '—'
      const wy = Number.isFinite(r.wy_mm) ? (r.wy_mm * wScale).toFixed(4) : '—'
        ...
        {!p ? (
          ...
          <>
            ...
            <div className="prop-header">
              ...
              <span className="prop-field" title="Show and enter beam size as the 1/e² intensity radius or diameter">
                Beam size
                <span className="prop-seg">
                  {['radius', 'diameter'].map(m => (
                    <button key={m} type="button"
                      className={(p.widthMode ?? 'radius') === m ? 'active' : ''}
                      onClick={() => mutate({ widthMode: m })}>
                      {m === 'radius' ? 'Radius' : 'Diameter'}
                    </button>
                  ))}
                </span>
              </span>
              ...
              {p.source && (
                <>
                  ...
                  <button className="small-btn" onClick={reimport}
                    title="Refresh distances and focal lengths from the currently loaded project (upload an updated project .zip first)">
                    ↻ Reimport
                  </button>
                </>
              )}
            ...
            <div className="prop-section">
              <div className="prop-section-title" style={{ display: 'flex', alignItems: 'center' }}>
                ...
                <small style={{ marginLeft: 8, color: 'var(--text-muted)' }}>
                  · {wSym} is the 1/e² intensity {wName} in mm; div is the {wName === 'diameter' ? 'full-angle' : 'half-angle'} divergence
                </small>
                <span style={{ flex: 1 }} />
                <button className="small-btn" onClick={() => setFitModalOpen(true)}
                  title="Fit waist size and position from a set of beam-width measurements">
              ...
              <div className="prop-section-body">
                <div className="prop-row" style={{ marginBottom: 4 }}>
                  <label className="prop-field">Specify by
                    <select className="snap-input" style={{ width: 200 }}
                      value={p.initMode ?? 'beam'}
                      onChange={e => mutate({ initMode: e.target.value })}>
                      <option value="beam">{`Local ${wName} + divergence`}</option>
                ...
                {(p.initMode ?? 'beam') === 'waist' ? (
                  <div className="prop-row">
                    <label className="prop-field">{wSym}₀{p.splitXY ? ' (x)' : ''} (mm)
                      <NumberField style={{ width: 90 }} step="0.01"
                        value={p.wxWaist_mm * wScale} fallback={DEFAULT_W0_MM * wScale}
                        onCommit={v => mutate({ wxWaist_mm: v / wScale })} /></label>
                    ...
                    {p.splitXY && (
                      <>
                        <label className="prop-field">{wSym}₀ (y) (mm)
                          <NumberField style={{ width: 90 }} step="0.01"
                            value={p.wyWaist_mm * wScale} fallback={DEFAULT_W0_MM * wScale}
                            onCommit={v => mutate({ wyWaist_mm: v / wScale })} /></label>
                  ...
                  <div className="prop-row">
                    <label className="prop-field">{wSym}{p.splitXY ? ' (x)' : ''} (mm)
                      <NumberField style={{ width: 90 }} step="0.01"
                        value={p.w0x_mm * wScale} fallback={DEFAULT_W0_MM * wScale}
                        onCommit={v => mutate({ w0x_mm: v / wScale })} /></label>
                    <label className="prop-field">div{p.splitXY ? ' (x)' : ''} (mrad)
                      <NumberField style={{ width: 80 }} step="0.1"
                        value={p.divx_mrad * wScale} fallback={0}
                        onCommit={v => mutate({ divx_mrad: v / wScale })} /></label>
                    {p.splitXY && (
                      <>
                        <label className="prop-field">{wSym} (y) (mm)
                          <NumberField style={{ width: 90 }} step="0.01"
                            value={p.w0y_mm * wScale} fallback={DEFAULT_W0_MM * wScale}
                            onCommit={v => mutate({ w0y_mm: v / wScale })} /></label>
                        <label className="prop-field">div (y) (mrad)
                          <NumberField style={{ width: 80 }} step="0.1"
                            value={p.divy_mrad * wScale} fallback={0}
                            onCommit={v => mutate({ divy_mrad: v / wScale })} /></label>
                      </>
                    )}
                  </div>
                )}
            ...
            <div className="prop-section">
              <div className="prop-section-title" style={{ display: 'flex', alignItems: 'center' }}>
                <span>Optics and test points</span>
                <span style={{ flex: 1 }} />
                <button className="small-btn" style={{ textTransform: 'none', letterSpacing: 0 }}
                  onClick={() => projectZipInputRef.current.click()}
                  title="Choose a designer project .zip, then pick a beam path to import as a new propagation. Propagations inside the zip are ignored.">
                  Upload from project .zip
                </button>
              </div>
              <div className="prop-section-body">
                <div className="prop-actions">
                  ...
                  <button className="small-btn" onClick={addTestPoint} style={{ marginLeft: 6 }}
                    title="Mark a z position where the beam size is reported">
                    + Add test point
                  </button>
                  {passthroughCount > 0 && (
                ...
                <table className="prop-table">
                  <thead>
                    <tr>
                      <th title="Include this optic in the propagation (for a test point: show it on the plot)">On</th>
                      <th>Role</th><th>z (mm)</th><th>f / M</th><th>Label</th>
                      <th>Element</th><th></th>
                    </tr>
                  </thead>
                  <tbody>
                    {visibleOptics.map(([o, i]) => {
                      ...
                      const isTest  = o.kind === 'test'
                      const enabled = o.enabled !== false
                      // Combined "Role" value: lens-spherical / lens-cylX /
                      // lens-cylY / prism-x / prism-y / test / passthrough.
                      // Reads current kind+shape and writes both on change.
                      const role = isPrism
                      ...
                      function setRole(next) {
                        ...
                        else if (next === 'test')      mutateOptic(i, { kind: 'test' })
                      ...
                      const rowClass =
                        [!enabled ? 'disabled-optic' : '',
                         (isLens || isPrism || isTest) ? '' : 'passthrough']
                        .filter(Boolean).join(' ')
                      return (
                        <tr key={o.id} className={rowClass}
                          ...
                          <td style={{ textAlign: 'center' }}>
                            <input type="checkbox" checked={enabled}
                              title={isTest
                                ? (enabled ? 'Hide this test point from the plot' : 'Show this test point on the plot')
                                : (enabled ? 'Disable — skip this optic in the propagation'
                          ...
                          <td>
                            <select className="snap-input" style={{ minWidth: 150 }}
                              ...
                              <option value="test">Test point</option>
                    ...
                    {(!p.optics || !p.optics.length) && (
                      <tr><td colSpan={7} className="dim" style={{ textAlign: 'center', padding: 8 }}>
                        No optics or test points — add one above or import from a beam path.
                      </td></tr>
                    )}
                  </tbody>
                </table>
                  ...
                  <table className="prop-table">
                    <thead>
                      <tr>
                        ...
                        <th>f / M</th>
                        <th>{wSym}{p.splitXY ? '_x' : ''} (mm)</th>
                        {p.splitXY && <th>{wSym}_y (mm)</th>}
                        <th>Element</th>
                      </tr>
                    </thead>
                    <tbody>
                      {compute.rows
                        ...
                        return (
                          <tr key={i} className={r.kind === 'passthrough' ? 'passthrough' : ''}
                            ...
                            <td>{valCell}</td>
                            <td>{Number.isFinite(r.wx_mm) ? (r.wx_mm * wScale).toFixed(4) : '—'}</td>
                            {p.splitXY && <td>{Number.isFinite(r.wy_mm) ? (r.wy_mm * wScale).toFixed(4) : '—'}</td>}
